...
public:
	...
	WidgetAnimation( Widget* widgetToAnimate, Rgba8 const& finalTint, float animationTime, eSmoothingFunction smoothingFunction );
	~WidgetAnimation();

	void SetStartingTransform( Transform const& startingTransform );
	void Update( float deltaSeconds );
	void UpdateWidgetTransform();
	void UpdateWidgetTint();
